...
import { Mail, Phone, Building2, MapPin, Calendar, Loader2, NotebookPen, ListTodo } from 'lucide-react';
...
export function ContactDetailsModal({ isOpen, onClose, contact }: ContactDetailsModalProps) {
    ...
    const { detail, isLoading, error, createTask, createNote, updateTask } = useCrmRecordDetails('contacts', contact?.id);
    const [taskTitle, setTaskTitle] = useState('');
    const [noteBody, setNoteBody] = useState('');

    if (!contact) return null;

    const contactRecord = (detail ?? contact) as Record<string, any>;
    ...
    const linkedDeals = Array.isArray(detail?.deals)
        ? detail.deals as Array<{ id: string; name: string; valueAmount?: number | null; stage?: { name?: string | null } | string | null }>
        : [];
    const contactName = contact.name || `${contactRecord.firstName || ''} ${contactRecord.lastName || ''}`.trim() || contactRecord.email || 'Unnamed contact';
    const contactLocation = contactRecord.country || contact.location || 'N/A';
    const referenceScenario = readReferenceScenario(contactRecord);
    const companyLabel = linkedCompany?.name || contact.company || 'N/A';

    const handleCreateTask = async () => {
        if (!taskTitle.trim()) return;
        try {
            await createTask({ title: taskTitle.trim(), contactId: contact.id });
            setTaskTitle('');
            toast({ title: 'Task created', description: 'The task was linked to this contact.' });
        } catch (err) {
    ...
    const handleCreateNote = async () => {
        ...
        try {
            await createNote({
                subject: `Note added for ${contact.name || contact.email}`,
                body: noteBody.trim(),
                contactId: contact.id,
            });
            setNoteBody('');
            toast({ title: 'Note added', description: 'The note was saved for this contact.' });
        } catch (err) {
    ...
    const handleToggleTask = async (taskId: string, currentStatus: 'pending' | 'completed') => {
        try {
            ...
            toast({
                ...
                description: 'The contact task status was updated.',
            });
        } catch (err) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="!top-[4vh] !translate-y-0 flex !h-[92vh] w-[96vw] max-w-[920px] flex-col overflow-hidden p-0 sm:w-full">
                ...
                <div className="min-h-0 flex-1 overflow-y-auto px-6 py-4 overscroll-contain">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-muted-foreground">{contactRecord.jobTitle || contact.jobTitle || 'No role specified'}</p>
                            <div className="mt-2 flex flex-wrap gap-2">
                                <Badge className="mt-0" variant={contact.status === 'Active' ? 'success' : 'secondary'}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <DetailField icon={<Mail className="h-4 w-4 text-muted-foreground" />} label="Email" value={contactRecord.email || contact.email} />
                        <DetailField icon={<Phone className="h-4 w-4 text-muted-foreground" />} label="Phone" value={contactRecord.phone || contact.phone || 'Not provided'} />
                            ...
                            {linkedCompany?.id ? (
                                <Button
                                    ...
                                    >
                                        ...
                                        {linkedCompany.name || companyLabel || 'Open company'}
                                    </Button>
                                ) : (
                        ...
                        <DetailField icon={<MapPin className="h-4 w-4 text-muted-foreground" />} label="Location" value={contactLocation} />
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <SectionCard
                            ...
                            content={(
                                <div className="space-y-3">
                                    ...
                                    {tasks.length === 0 ? (
                                        <p className="text-sm text-muted-foreground">No tasks linked to this contact yet.</p>
                                    ) : (
                        ...
                        <SectionCard
                            ...
                            content={(
                                <div className="space-y-3">
                                    <Textarea value={noteBody} onChange={(e) => setNoteBody(e.target.value)} placeholder="Write a note about this contact" />
                        ...
                        <SectionCard
                            ...
                            content={(
                                linkedDeals.length === 0 ? (
                                    <p className="text-sm text-muted-foreground">No deals linked to this contact yet.</p>
                                ) : (
                                    <div className="space-y-2">
                    ...
                    <SectionCard
                        ...
                        content={(
                            recentActivity.length === 0 ? (
                                <p className="text-sm text-muted-foreground">No system activity recorded for this contact yet.</p>
                            ) : (
                                <div className="space-y-2">
                    ...
                    <SectionCard
                        ...
                        content={(
                            auditLogs.length === 0 ? (
                                <p className="text-sm text-muted-foreground">No audited changes yet for this contact.</p>
                            ) : (
                                <div className="space-y-2">
                                    {auditLogs.map((log) => (
                    ...
                    <div className="pt-4 border-t border-border">
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            ...
                            Added on {new Date((detail?.createdAt as string) || contact.createdAt || Date.now()).toLocaleDateString()}
                        </div>
...
function readReferenceScenario(contactRecord: Record<string, any>) {
    return contactRecord?.customFieldsJson?.referenceScenario || null;
}
