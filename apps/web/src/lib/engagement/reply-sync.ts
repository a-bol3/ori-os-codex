...
const DEFAULT_IMAP_PASSWORD = process.env.IMAP_PASSWORD || process.env.SMTP_PASSWORD || "";
const DEFAULT_IMAP_SECURE =
    (process.env.IMAP_SECURE || "").toLowerCase() === "true" || DEFAULT_IMAP_PORT === 993;
...
function isHeaderCollection(value: unknown): value is HeaderCollection {
    return !!value && typeof value === "object" && "get" in value && typeof (value as { get?: unknown }).get === "function";
}
...
export async function syncReplySignalsForOrganization(
    ...
    if (!organizationId || !DEFAULT_IMAP_USER || !DEFAULT_IMAP_PASSWORD || !DEFAULT_IMAP_HOST) {
    ...
    const sentByMessageId = new Map<
        string,
        { campaignId: string | null; contactId: string | null; mailboxId: string | null }
    >();
    ...
    try {
        ...
        for await (const message of client.fetch(recentUids, {
            ...
            const headers = isHeaderCollection(message.headers) ? message.headers : undefined;
            ...
            const references = directReplyReference
                ...
                : referencesHeader
                      ...
                      .filter(Boolean);
            const matchedReference = references.find((reference) => sentByMessageId.has(reference));
            ...
            await prisma.$transaction(async (tx: any) => {
                const openedAlreadyExists = await tx.emailEvent.findFirst({
                    where: {
                        campaignId: sentEvent.campaignId!,
                        contactId: sentEvent.contactId!,
                        eventType: "OPENED",
